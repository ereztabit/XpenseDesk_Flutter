...
- [x] Manager can edit/delete/approve any expense unless the sheet is Approved (escape hatch to fix + re-approve on behalf of an unresponsive employee). DONE: client shipped across all layouts (detail screen, desktop table, mobile compact list, mobile swipe card), Approved sheets fully locked, manager can approve/delete a declined line directly on a Declined sheet; server procs applied (proc_UpdateExpense, proc_DeleteExpense, proc_ApproveExpense, proc_EvaluateExpenseSheet). Remaining: backend test suite verification (backend agent). See docs/bugs/completed/manager-edit-delete-non-approved-sheets.md
