...
- [ ] [Business][P1] **FS-1004** Manager adds an expense to an employee's sheet -- an "Add expense" action in Sheet Review for a WaitingForApproval or Declined sheet only: the manager files the line the employee could not (usually the missing receipt), the line belongs to the **employee** (they are reimbursed) and is created **already approved** (the manager entering it is the one who would approve it), so it lands in the Approved tab and the sheet's own status does not change. Form must name the employee it is filed under and the target cycle (usually a previous one). Extracted from FS-1003 (2026-09-05) as the small, standalone half. **Blocked on the backend half** (no `expenseSheetId` on create). see docs/backlog/manager-adds-expense-to-employee-sheet-spec.md
