...
- For managers, there is no "reopen" anymore; if a manager wants to undo a decision, they need to wait for the employee to re-submit or contact the employee directly.
  - **Update (2026-06-11):** superseded for the whole-sheet case — `POST /api/expense-sheets/{id}/approve` now also accepts `Declined` sheets (declined lines stay declined; all-declined sheets close as Approved with nothing reimbursed). Re-decline is still rejected. See docs/completed/manager-reapprove-declined-sheet.md.
